...
const VariablesQuiz = () => {
    ...
    return (
        <div>
            ...
            {previousScore !== null && (
                <p>Your Previous Score: {previousScore} / {questions.length}</p>
            )}
            ...
            {submitted && (
                <div className="appreciation-message">
                    ...
                    <button onClick={retakeQuiz}>Retake Test</button>
                    <button onClick={handleBackToMainPage}>Back to Main Page</button> {/* Back to Main Page button */}
                </div>
            )}
        </div>
    );
